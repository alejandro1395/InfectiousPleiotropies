...
mkdir -p ${INPUT}Infec_pleios/
mkdir -p ${INPUT}Infec_snps/
...
'SELECT DISTINCT SNPA,DiseaseA,RiskAllA,OnsetA,SNPB,DiseaseB,RiskAllB,onsetB,ID,CHR FROM filteredPairs WHERE R2 >= 0.8 AND (GroupA IN ("infectious disease") 
 OR GroupB IN ("infectious disease")) AND CHR != "" ;' > ${INPUT}Infec_pleios/pleios.txt

### Construct haplotypes with plink
cat ${INPUT}Infec_pleios/pleios.txt | while read line; do
    snpA=$(echo "$line" | cut -f 1);
    chr=$(echo "$line" | cut -f 10);
    echo $snpA;
    snpB=$(echo "$line" | cut -f 5);
    echo -e '*' ${snpA}'\t'${snpB} > ${INPUT}Infec_snps/snps.hlist;
    sed -i -e 's/ /\t/g' ${INPUT}Infec_snps/snps.hlist;
    ${BIN}plink --file ${VCF}chr${chr}_CEU_genotypes \
--hap ${INPUT}Infec_snps/snps.hlist \
--hap-freq \
--noweb \
--out ${INPUT}Infec_snps/${snpA}_${snpB};
grep -v LOCUS ${INPUT}Infec_snps/${snpA}_${snpB}.frq.hap | awk '{print $2,$3}' > ${INPUT}Infec_snps/${snpA}_${snpB}.fhtp
done
...
### Python script to count pleiotropies in haplotypes
	python3 ./countPleiotropies.py ${INPUT}Infec_snps/${snpA}_${snpB}.fhtp ${riskHap} ${line} ${INPUT}Infec_snps/; ##### poner ${arx} para hacerlo como el original DONE. 	
    done
